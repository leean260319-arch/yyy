...
Author: oais
Date: 2026-01-29
Description: data/converted_jpg/의 식품 뒷면 이미지들을 OCR로 JSON으로 변환
"""
...
# 설정
INPUT_DIR = Path("data/converted_jpg")
OUTPUT_DIR = Path("data/food_info_json")
LANGUAGES = ["ko", "en"]
...
def run_ocr(reader, image_path):
    ...
    avg_confidence = (
        round(total_confidence / len(ocr_results), 4) if ocr_results else 0.0
    )
...
def process_single_image(reader, image_path):
    ...
    # 결과 저장
    output_filename = image_path.stem + "_ocr_result.json"
    output_path = OUTPUT_DIR / output_filename
    save_result(result, output_path)

    print(
        f"[처리 완료] {image_path.name} - 인식된 텍스트: {result['total_texts']}개, 평균 신뢰도: {result['avg_confidence']:.2%}"
    )
...
def main():
    ...
    # 이미지 파일 수집 (01.jpg ~ 15.jpg)
    image_files = []
    for i in range(1, 16):
        file_path = INPUT_DIR / f"{i:02d}.jpg"
        if file_path.exists():
            image_files.append(file_path)
        else:
            print(f"[경고] 파일 없음: {file_path.name}")
    ...
    overall_avg_confidence = (
        sum(r["avg_confidence"] * r["total_texts"] for r in all_results) / total_texts
        if total_texts > 0
        else 0.0
    )
