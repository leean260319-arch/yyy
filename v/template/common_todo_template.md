...
> **디버깅 가이드**: v/guide/debugging_guide.md 참조 (워크플로우, 에러 분류, False Positive 처리)
...
> 1~2주 모니터링 후 oaishistory run → d{SP}0010_history.md 아카이브
...
> `oaistodo add` 명령으로 추가된 항목. 완료 시 삭제 또는 해결된 이슈로 이동.
...
1. 이 템플릿을 복사하여 `doc/d{SP}0004_todo.md` 생성
...
| 명령어 | 용도 |
|--------|------|
| `oaischeck` | 코드 분석 후 이슈 등록 |
| `oaisfix` | d0004 기반 이슈 수정 |
| `oaishistory` | 해결된 이슈 → d0010 아카이브 |

---

## 참고 문서

- 이슈 등록 템플릿: `v/template/issue_template.md`
- 에러 리포트 템플릿: `v/template/error_report.md`
- 디버깅 가이드: `v/guide/debugging_guide.md`
- 공통 가이드: `v/guide/common_guide.md` (섹션 6.2 참조)
